...
impl Connection {
    ...
    pub async fn to(socket: impl ToSocketAddrs) -> Result<Self, RRError> {
        Ok(Self {
            socket: TcpStream::connect(socket)
                .await
                .map_err(|_| RRErrorKind::NetworkError(NetworkErrorKind::ConnectionFailed))?,
        })
    }
    ...
    pub async fn sendrecv(
        &mut self,
        frame: Frame<impl Into<String> + Display>,
    ...
    pub async fn write_frame<T: Into<String> + Display>(
        &mut self,
        frame: Frame<T>,
    ) -> Result<(), RRError> {
        ...
        self.socket
            ...
            .await
            .map_err(|_| RRErrorKind::NetworkError(NetworkErrorKind::FrameWriteError))?;
        Ok(())
    }
...
where {
    ...
    async fn advance_stream(&mut self) -> Result<usize, RRError> {
        ...
        Err(RRErrorKind::SerializationError(SerializationErrorKind::FormatError).into())
    }
}
